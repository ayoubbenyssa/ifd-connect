...
class CommentReplies extends StatefulWidget {
  CommentReplies(this.comment, this.user_me, {Key key}) : super(key: key);
  Comment comment;
  User user_me;
...
class _CommentRepliesState extends State<CommentReplies> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
              ...
              children: [
                ...
                Container(
                    ...
                    child: ClipRRect(
                        ...
                        child: widget.comment.post.pic.toString() != "null" &&
                                widget.comment.post.pic.toString() != "[]"
                            ? Image.network(
                                widget.comment.post.pic[0],
                ...
                Column(
                  ...
                  children: [
                    Container(
                        ...
                        child: Container(
                            child: Text(widget.comment.post.name.toString(),
                                maxLines: 2,
                    ...
                    Container(
                        width: MediaQuery.of(context).size.width * 0.5,
                        child: HtmlWidget(
                          widget.comment.post.description.toString().length >
                                  140
                              ? widget.comment.post.description
                                      .toString()
                                      .substring(0, 140) +
                                  '..'.replaceAll(RegExp(r'(\\n)+'), '')
                              : widget.comment.post.description
        ...
        body: ListView(
          children: [
            MyCommenntItem(widget.comment, widget.user_me),
          ],
        ));
  }
}

class MyCommenntItem extends StatefulWidget {
  MyCommenntItem(this.comment, this.user_me, {Key key}) : super(key: key);
  Comment comment;
  User user_me;

  @override
  _MyCommenntItemState createState() => _MyCommenntItemState();
}

class _MyCommenntItemState extends State<MyCommenntItem> {
  ...
  _postComment(String message) async {
    ...
    final result = await commentsRepository.postReply(
      widget.comment.id,
      postId: widget.comment.post.objectId,
      content: _comment,
    );


    print("---------------------");
    print(widget.comment.author1.fullname);
