...
*/
  ...
  putparse(geturl, data) async {
    ...
    if (con) {
      ...
      var url = Uri.parse(Uri.encodeFull(urlclasse + geturl));
      var response =

      await clientHttp.put(url, headers: paramsparsejson(), body: data);
  ...
  getparselogin(geturl) async {
    ...
    if (con) {
      ...
      var response = await clientHttp.get(url, headers: paramsparsenojson());
      String jsonBody = response.body;
  ...
  getparse(geturl) async {
    ...
    if (con) {
      ...
      try {
        response = await clientHttp.get(url, headers: paramsparsenojson());
      }
      catch(e)
      {
  ...
  deleteparse(geturl) async {
    ...
    if (con) {
      var url = Uri.parse(Uri.encodeFull(urlclasse + geturl));
      var response = await clientHttp.delete(url, headers: paramsparsenojson());
      String jsonBody = response.body;
      var statusCode = response.statusCode;
      if (statusCode < 200 || jsonBody == null || statusCode >= 300) {
        results = "error";
