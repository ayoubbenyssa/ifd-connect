...
class Confirmation_password extends StatefulWidget {
  Confirmation_password();

  @override
  _Confirmation_passwordState createState() => _Confirmation_passwordState();
}

class _Confirmation_passwordState extends State<Confirmation_password> {
  ...
  FocusNode _focus_newpasswprd = new FocusNode();
  FocusNode _focus_newpasswprd_confirm = new FocusNode();

  final _new_passwordcontroller = new TextEditingController();
  final _new_password_confirmcontroller = new TextEditingController();
  ...
  void _handleSubmitted() async {
    ...
    if (!form.validate()) {
    ...
    } else if(_new_password_confirmcontroller.text != _new_passwordcontroller.text){
      showInSnackBar("La confirmation ne correspond pas à votre nouveau mot de passe");
    }
    else {
      setState(() {
        _authHint = '';
      ...
      final batchData = await http.post(
        "${"${prefs.getString('api_url')}"}/forgot_password",
        body: param,
      );
...
*/
  ...
  Widget build(BuildContext context) {
    ...
    Widget New_password =  TextFieldWidget(
      "Nouveau mot de passe",
      _focus_newpasswprd,
      _new_passwordcontroller,
      TextInputType.text,
      val.validateid_password,
      obscure: true,
      suffixIcon: "assets/icons/Lock.svg",
    ) ;
    Widget Confirmation_new_password =  TextFieldWidget(
      "Confirmation le nouveau mot de passe",
      _focus_newpasswprd_confirm,
      _new_password_confirmcontroller,
      TextInputType.text,
      val.validateid_password,
      obscure: true,
      suffixIcon: "assets/icons/Lock.svg",
