...
class _MesReservationState extends State<AllReservations> {
  ...
  Future<http.Response> makeCancel(
      int user_id, int student_id, int token, String ticketId,dateModif) async {
    ...
    final param = {
      ...
      "resto_api_check_date": "$dateModif"
    };

    final reservationsData = await http.post(
      "${"${prefs.getString('api_url')}"}/make_cancellation",
      body: param,
    );
    setState(() {
...
// if(date!=null){
  ...
  Future<http.Response> getMealsFromAPi(int user_id, int student_id, int token,
    ...
    final mealsData = await http.post(
      "${"${prefs.getString('api_url')}"}/api/all_meals_calendar",
      body: param,
    );
